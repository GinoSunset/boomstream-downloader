...
class App:
    ...
    def get_token(self):
        if (
            "records" in self.config["mediaData"]
            and len(self.config["mediaData"]["records"]) > 0
        ):
            return b64decode(self.config["mediaData"]["records"][0]["token"]).decode(
                "utf-8"
            )
        else:
    ...
    def get_m3u8_url(self):
        if (
            ...
            return b64decode(
                self.config["mediaData"]["records"][0]["links"]["hls"]
            ).decode("utf-8")
        else:
    ...
    def get_boomstream_config(self, page):
        ...
        for script in html.xpath('//script[@type="text/javascript"]'):
            m = re.search(
                "window.boomstreamConfig = ({.*});$", script.text_content(), flags=re.M
            )
            if m is not None:
            ...
            if line.startswith("#EXT-X-STREAM-INF"):
                ...
                if m is not None:
                    ...
                    if m is not None:
                        ...
                        raise Exception(
                            "Could not get resolution from EXT-X-STREAM-INF"
                        )
    ...
    def get_chunklist(self, playlist):
        ...
        print(
            "This video is available in the following resolutions: %s"
        ...
        if self.args.resolution is not None:
            ...
            if url is None:
                raise Exception(
                    "Playlist for resolution specifeid is --resolution "
    ...
    def get_aes_key(self, xmedia_ready):
        ...
        key_url = "https://play.boomstream.com/api/process/" + self.encrypt(
            decr[0:20] + self.token, XOR_KEY
        )
    ...
    def download_chunks(self, chunklist, iv, key):
        ...
        chunks = [i for i in chunks if i.startswith("https://")]
        for line in chunks:
            outf = os.path.join(self.title, "%0.5d" % i) + ".ts"
            if os.path.exists(outf):
                i += 1
                print(f"Chunk #{i} exists [{outf}]")
                continue
            print(f"Downloading chunk #{i}/{len(chunks)-1}")
            os.system(
                'curl -s "%s" | openssl aes-128-cbc -K "%s" -iv "%s" -d > %s'
    ...
    def merge_chunks(self, key):
        ...
        self.encoding_to_mp4()

    def encoding_to_mp4(self):
        print("Encoding to MP4")
        os.system(f'ffmpeg -i {self.title}.ts -c copy "{self.title}".mp4')
